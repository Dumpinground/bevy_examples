...
fn main() {
    ...
    .add_systems(Startup, (save_scene_system, load_scene_system, infotext_system))
    .add_systems(Update, log_system)
    .run();
}

// Registered components must implement the `Reflect` and `FormWorld` traits.
// The `Reflect` trait enables serialization, deserialization, and dynamic property access.
// `Reflect` trait enable a bunch of cool behaviors, so it's worth checing out the dedicated `reflect.rs`
...
fn save_scene_system(world: &mut World) {
    ...
    // To do this, we can simplu clone the `AppTypeRegistry` resource.
    ...
    scene_world.spawn(ComponentA { x: 3., y: 4. });
    scene_world.insert_resource(ResourceA { score: 1 });

    // With our sample world ready to go, we can now create out scene using DynamicScene or DynamicSceneBuilder.
...
// This is only necessary for the info message in the UI. See examokes/ui/text.rs for a standalone
// text example.
